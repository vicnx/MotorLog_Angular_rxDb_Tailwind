...
export class UtilsService {
	...
	static isServiceOrphan(serviceValue: any, allOptions: any[]): boolean {
		if (!allOptions || !serviceValue) return false;

		const option = allOptions.find((opt) => opt.value === serviceValue);
		return !option || option.isOrphan === true;
	}
}
